    ...
    <aside className="w-64 bg-white/90 border-r border-slate-100 min-h-[calc(100vh-61px)] p-4 flex flex-col justify-between hidden md:flex shrink-0 text-left font-sans">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between px-3 mb-2">
          <p className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-1.5 font-montserrat">
            <Layers className="h-3.5 w-3.5 text-amber-600" /> {currentWorkspace.workspaceTitle}
          </p>
          {effectiveRole === 'CEO' ? (
            <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 border border-amber-300 font-apfel" suppressHydrationWarning>
              CEO SUPER ADMIN
            </span>
          ) : (
            <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-sky-100 text-sky-800 border border-sky-300 uppercase font-apfel" suppressHydrationWarning>
              {effectiveRole}
            </span>
          )}
        </div>
        ...
        <div className="space-y-5">
          {currentWorkspace.categories.map((category) => (
            <div key={category.title} className="space-y-1">
              <p className="font-montserrat text-[10px] font-extrabold uppercase tracking-widest text-slate-400 px-3.5 mb-1.5">
                {category.title}
              </p>
              ...
              <div className="space-y-1">
                {category.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 truncate">
                        <Icon className={`h-4 w-4 shrink-0 ${active ? 'text-[#d97706]' : item.color || 'text-slate-500'}`} />
                        <span className="truncate">{item.name}</span>
                      </div>
                      ...
                      {item.badge ? (
                        ...
                        <ChevronRight className={`h-3.5 w-3.5 shrink-0 ${active ? 'text-[#d97706]' : 'text-slate-300'}`} />
                      )}
                    </Link>
                  );
                })}
